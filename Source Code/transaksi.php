...
if (isset($_POST['kirim']) ){
    if(datadua($_POST) > 0) {
        echo "<script>alert('DATA PEMASUKAN BERHASIL DI INPUT')</script>";
    }
    else{
        echo "<script>alert('DATA PEMASUKAN GAGAL DI INPUT')</script>";
    }
}

$koneksi = mysqli_connect("localhost","root","","dikoin");

$result = mysqli_query($koneksi, "SELECT * FROM datapemasukan") ;
...
<div class="container">
    ...
    <br>
        ...
        <form action="transaksi.php" method="post">
            <div class="card-body" style="background: #519F5E; border-radius: 15px;">
                <div class="row mb-3">
                ...
                <div class="input-group mb-3">
                    ...
                    <select name="kategori" class="form-select" id="inputGroupSelect01" style="padding-left: 15px; border-radius: 5px;">
                        ...
                        <option name="kategori" required="required" value="gaji">Gaji atau Uang Saku</option>
                        <option name="kategori" required="required" value="pendapatanpasif">Bisnis atau Pendapatan Pasif</option>
                        <option name="kategori" required="required" value="bunga">Bunga atau Investasi</option>
                        <option name="kategori" required="required" value="incomelainnya">Pendapatan Lainnya</option>
                    </select>
                </div>
                          <div class="text-end">
                              <button class="btn btn-primary" type="submit"  value="Kirim" name="kirim" style="background: #005F0F; border-radius: 10px; border: 0px; font-family: 'Baloo 2', cursive;" > Perbarui Pemasukan</button>
                          </div>
            </div>
        </form>
</div>        
